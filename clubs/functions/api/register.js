...
export async function onRequestPost(context) {
  ...
  try {
    ...
    return json({message: 'Invalid registration data.'}, 400);
  }
  ...
  if (!context.env.CLUBS_PORTAL_REGISTER_URL) {
    return json({message: 'The registration page is ready, but the portal connection has not been configured yet.'}, 503);
  }

  const headers = {'content-type': 'application/json'};
  if (context.env.CLUBS_PORTAL_TOKEN) {
    headers.authorization = `Bearer ${context.env.CLUBS_PORTAL_TOKEN}`;
  }
  ...
  try {
    ...
    if (!upstream.ok) {
      return json({
        message: data.message || data.error || 'The portal could not complete this registration.'
      }, upstream.status >= 400 && upstream.status < 600 ? upstream.status : 502);
    }

    return json({
      ok: true,
      message: data.message || 'Registration received.',
